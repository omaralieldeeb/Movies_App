...
class HomeRepoImpl implements HomeRepo{
  ...
  Future<Either<Failure, List<MovieModel>>> fetchAllPosters()async {
    try {
      var data = await apiService.get(endpoint:'trending/tv/day?language=en-US&api_key=c3832ae8a72163d620520865e6283997');
  ...
  Future<Either<Failure, List<MovieModel>>> fetchLatestMovies()async {
    try {
      var data = await apiService.get(endpoint: 'movie/upcoming?api_key=c3832ae8a72163d620520865e6283997&include_adult=false&page=2');
      List<MovieModel> movies = [];
      for(var i in data['results']){
        movies.add(MovieModel.fromJson(i));
      }
  ...
  Future<Either<Failure, List<MovieModel>>> fetchPopularMovies()async {
    try {
      var data = await apiService.get(endpoint: 'movie/popular?api_key=c3832ae8a72163d620520865e6283997&include_adult=false');
      List<MovieModel> movies = [];
      for(var i in data['results']){
        movies.add(MovieModel.fromJson(i));
      }
  ...
  Future<Either<Failure, List<MovieModel>>> fetchSimilarMovies({required int id}) async{
    try {
      var data = await apiService.get(endpoint: 'movie/$id/similar?api_key=c3832ae8a72163d620520865e6283997&include_adult=false');
      List<MovieModel> movies = [];
      for(var i in data['results']){
        movies.add(MovieModel.fromJson(i));
      }
  ...
  Future<Either<Failure, List<MovieModel>>> fetchCategoryMovies({required String query}) async{
    try {
      var data = await apiService.get(endpoint: 'search/movie?query=$query&include_adult=false&page=1&api_key=c3832ae8a72163d620520865e6283997');
      List<MovieModel> movies = [];
      for(var i in data['results']){
        movies.add(MovieModel.fromJson(i));
      }
